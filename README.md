...
`delivery_id` совпадает с `payment_id` и стабилен между повторными попытками —
это ключ дедупликации на стороне получателя, см. [доставку at-least-once](#доставка-webhook-at-least-once).
...
### Доставка webhook: at-least-once

Consumer отправляет webhook **до** `ack`, поэтому падение процесса между
отправкой и подтверждением приведёт к повторной доставке сообщения из очереди.
Два уровня защиты гасят этот сценарий:

* платёж уже обработан → `process_created` возвращает `already_processed`,
  и consumer вообще не идёт в доставку;
* `deliver` отказывается работать, если `webhook_status` уже терминальный
  (`delivered` или `failed`).

Одно окно остаётся принципиально незакрываемым: если процесс умрёт **после**
успешного HTTP-запроса, но **до** коммита `webhook_status = delivered`, строка
останется в `pending`, и воркер `webhook-retry` отправит уведомление повторно.
Закрыть его без распределённой транзакции между HTTP и БД нельзя, поэтому
контракт такой: **получатель webhook обязан быть идемпотентным**.

Ключ дедупликации — `delivery_id` в теле уведомления. Он стабилен для всех
попыток доставки одного платежа, так что получателю достаточно хранить
обработанные `delivery_id` и игнорировать повторы.
